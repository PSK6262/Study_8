...
public class Cookie01Controller {
	...
	public String saveCookie(HttpServletResponse response) {
		...
		try {
			...
			Cookie ck4 = new Cookie("status", URLEncoder.encode("상당히 배고프고 졸림","UTF-8"));
			ck4.setMaxAge(60*60);
	...
	public String readCookie(HttpServletRequest request) {
		...
		for(Cookie ck : cookies) {
			...
			String temp = MyCookieUtil.getCookieValue(request, "temp");
			String status = MyCookieUtil.getCookieValue(request, "status");
