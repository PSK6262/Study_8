...
//날씨 정보 저장 상태관리
// let [weather, setWeather] = useState( {weather:"", temperatue:15, hmdt:40  } );
...
let weatherSlice = createSlice({
    ...
    reducers : {
        ...
        setWeatherInfo(state, action){  //state 현재 원본 상태값(객체라서 주소값), action 전달온 매개변수

            //action.payload.weather
            //action.payload.temp

            let { weather, temp, hdmt } = action.payload;
